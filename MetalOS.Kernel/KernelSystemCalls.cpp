...
uint64_t Kernel::Syscall(SystemCallFrame* frame)
{
	...
	case SystemCall::GetDesktop:
		return (uint64_t)GetDesktop();
	...
	case SystemCall::CreateDesktop:
		return (uint64_t)CreateDesktop((Handle*)frame->Arg0, (Point2D*)frame->Arg1);
...
SystemCallResult Kernel::CreateDesktop(Handle* handle, Point2D* bounds)
{
	...
	bounds->X = m_display->GetWidth();
	bounds->Y = m_display->GetHeight();

	UserThread* user = m_scheduler->GetCurrentUserThread();
	Assert(user);
	this->WindowThread = user;

	return SystemCallResult::Success;
}

HThread Kernel::GetDesktop()
{
	return kernel.WindowThread;
}
...
SystemCallResult Kernel::SetScreenBuffer(void* buffer)
{
	...
	UserThread* user = m_scheduler->GetCurrentUserThread();
	Assert(user);
	if (user->GetId() != this->WindowThread->GetId())
		return SystemCallResult::Failed;
